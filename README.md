...
- **Ingest anything**: YouTube videos (description + transcript → structured recipe), recipe websites (schema.org fast path + LLM fallback), manual entry, photos later. From iPhone via an Apple Shortcut, from any phone/PC via the paste box, and optionally from Android's share target after a future trusted-HTTPS upgrade.
