...
| D7 | Auth = named users + 400-day device-session cookies via magic-link/QR + separate Bearer ingest tokens; passwordless by default, with **optional per-user PIN sign-in** | Immich/HA-proven; survives iOS PWA storage isolation via the 17.2+ cookie copy. PIN sign-in added 2026-07-17 at owner request (scrypt-hashed, per-user lockout, still one `rc_session` cookie) — see CONVENTIONS §5/§6 |
